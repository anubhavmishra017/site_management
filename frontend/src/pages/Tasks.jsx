...
const Tasks = () => {
    ...
    try {
      ...
      const payload = {
        ...
        deadline: newTask.deadline
          ? formatDateForInput(newTask.deadline)
          : null,
      };
  ...
  // --------------------------
  // FIX: OVERDUE SHOULD IGNORE COMPLETED TASKS
  // --------------------------
  const overdueCount = tasks.filter((t) => {
    if (t.status === "Completed") return false; // FIX ADDED
  ...
  const matchesFilters = (t) => {
    if (filterWorker !== "All" && String(t.worker?.id) !== String(filterWorker))
      return false;
    if (filterProject !== "All" && String(t.project?.id) !== String(filterProject))
      return false;
  ...
  const getDeadlineState = (deadline, status) => {
    ...
    const today = new Date();
    const diffMs =
      dl - new Date(today.getFullYear(), today.getMonth(), today.getDate());
      ...
      <div className="bg-white rounded-xl shadow border overflow-auto">
        {loading ? (
          ...
          <table className="min-w-full table-auto">
            ...
            <tbody>
              {filteredTasks.map((t) => {
                const dlState = getDeadlineState(t.deadline, t.status); // 🔥 FIX ADDED
          ...
          <div className="bg-white rounded-xl p-6 w-[90%] max-w-lg relative">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm text-gray-700">
              <div>
                ...
                <div>{selectedTask.worker?.name || "-"}</div>
              </div>
              <div>
                <strong>Project</strong>
                <div>{selectedTask.project?.name || "-"}</div>
              </div>
              <div>
          ...
          <div className="bg-white rounded-xl shadow-xl p-6 w-[90%] max-w-md relative">
            ...
            <form className="space-y-4" onSubmit={handleSaveTask}>
              ...
              <select
                ...
                onChange={(e) =>
                  setNewTask({ ...newTask, workerId: e.target.value })
                }
                className="w-full border rounded-lg px-3 py-2"
                required
              >
